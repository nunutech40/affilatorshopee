...
<template>
  <section class="panel">
    ...
    <div class="form-grid"><div class="field"><label>Content model awal</label><select v-model="contentModel" class="select"><option value="">Belum ditentukan (bisa diisi AI)</option><option value="trending">Trending</option><option value="branded">Branded</option><option value="cheap">Murah</option><option value="curated">Curated</option></select></div><div class="field"><label>Tracking tag (opsional, untuk match komisi)</label><input v-model="trackingTag" :disabled="isXMode" class="input" :class="{ disabled: isXMode }" placeholder="misal: mytag01" /></div></div>
