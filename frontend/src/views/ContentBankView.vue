...
<template>
  <div class="content-page">
    ...
    <section class="panel"><div class="section-heading"><div><h2>Konten tersimpan</h2><p class="muted">{{ items.length }} konten pada tampilan ini · urutkan dan kurasi berdasarkan sinyal popularitas</p></div><div class="filter-row"><select v-model="selectedPlatform" class="select"><option value="">Semua sumber</option><option value="x">X</option><option value="threads">Threads</option><option value="facebook">Facebook</option></select><select v-model="selectedNiche" class="select"><option value="">Semua niche</option><option v-for="niche in niches" :key="niche.id" :value="niche.id">{{ niche.name }}</option></select><input v-model="search" class="input" placeholder="Cari konten..." /></div></div><p v-if="loading" class="muted">Memuat bank konten...</p><p v-if="error" class="error-box">{{ error }}</p><div v-else-if="items.length" class="content-list"><RouterLink v-for="item in items" :key="item.id" :to="`/content-bank/${item.id}`" class="content-card"><div class="content-card-head"><span class="card-tags"><span class="status-pill">{{ item.status }}</span><span class="source-pill">{{ item.platform }}</span><span v-for="niche in (item.niches || []).slice(0, 2)" :key="niche.id" class="niche-mini">{{ niche.name }}</span></span><span class="open-link">Detail →</span></div><h3>{{ itemTitle(item) }}</h3><p class="content-excerpt">{{ excerpt(item.original_text) }}</p><div class="popularity">{{ popularity(item) }}</div><div class="content-meta"><span>{{ item.author_handle || 'Author tidak dicatat' }}</span><span>{{ item.published_at ? new Date(item.published_at).toLocaleString('id-ID') : 'tanggal tidak dicatat' }}</span><span v-if="item.media?.length">{{ item.media.length }} media</span><span v-if="item.variants?.length">{{ item.variants.length }} varian</span></div></RouterLink></div><p v-else-if="!loading" class="empty-state">Belum ada konten. Mulai dari halaman tangkap konten.</p></section>
    <div v-if="items.length" class="content-delete-list">
      <strong>Hapus konten</strong>
      <button v-for="item in items" :key="`delete-${item.id}`" class="button-danger card-delete" type="button" @click="removeContent(item)">Hapus {{ itemTitle(item) }}</button>
    </div>
  </div>
</template>
...
<style scoped>
.content-delete-list{display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:12px 0;color:#52655a}.content-delete-list strong{margin-right:4px}.card-delete{padding:7px 10px;font-size:12px;line-height:1.2;max-width:260px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
</style>
