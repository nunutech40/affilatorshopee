...
const researchQueries = {
  'Sukses & Kesuksesan': ['cara sukses', 'kebiasaan orang sukses', 'karier dan produktivitas', 'bisnis dan keuangan'],
  'Fashion Pria': ['outfit pria', 'gaya pria', 'fashion pria', 'tips berpakaian pria'],
  'Hubungan / Relasi Pria Wanita': ['hubungan pria wanita', 'komunikasi dalam hubungan', 'kencan dan relasi', 'red flags hubungan'],
  'Gym, Lari & Exercise': ['gym pemula', 'lari dan olahraga', 'workout di rumah', 'tips latihan'],
  Affiliate: ['affiliate marketing', 'tips jualan online', 'shopee affiliate', 'konten jualan'],
}

const suggestedQueries = () => {
  const niche = niches.value.find((item) => item.id === researchNiche.value)
  return niche ? (researchQueries[niche.name] || [niche.name]) : []
}
...
function openResearch() {
  const query = researchQuery.value.trim() || suggestedQueries()[0] || ''
  if (!query) return
...
<template>
  <div class="content-page">
    ...
    <section class="panel research-form">
      ...
      <div class="form-grid"><select v-model="researchNiche" class="select"><option value="">Pilih niche konten</option><option v-for="niche in niches" :key="niche.id" :value="niche.id">{{ niche.name }}</option></select><input v-model="researchQuery" class="input" placeholder="Keyword opsional (sistem sudah siapkan)" @keyup.enter="openResearch" /></div>
      <div v-if="suggestedQueries().length" class="query-options"><button v-for="query in suggestedQueries()" :key="query" class="query-chip" type="button" @click="researchQuery = query">{{ query }}</button></div>
      <button class="button-primary" :disabled="!researchNiche && !researchQuery.trim()" @click="openResearch">Buka riset populer di X ↗</button>
      <p class="muted">Setelah menemukan post yang menarik di X, salin URL dan teksnya ke form di bawah. Integrasi capture extension akan ditambahkan pada tahap berikutnya.</p>
    </section>
