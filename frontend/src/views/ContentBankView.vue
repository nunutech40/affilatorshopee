...
async function removeContent(item) {
  if (!confirm(`Hapus konten "${itemTitle(item)}" dari bank?`)) return
  try {
    await apiRequest(`/api/content-items/${item.id}`, { method: 'DELETE' })
    await load()
...
function formatLabel(item) { return item.content_format === 'thread' ? 'Thread' : 'Post biasa' }
...
function itemTitle(item) {
  const firstLine = (item.original_text || '').split('\n').map((line) => line.trim()).find(Boolean) || 'Konten tanpa teks'
...
<template>
  <div class="content-page">
    ...
    <section class="panel"><div class="section-heading"><div><h2>Konten tersimpan</h2><p class="muted">{{ items.length }} konten pada tampilan ini · urutkan dan kurasi berdasarkan sinyal popularitas</p></div><div class="filter-row"><select v-model="selectedPlatform" class="select"><option value="">Semua sumber</option><option value="x">X</option><option value="threads">Threads</option><option value="facebook">Facebook</option></select><select v-model="selectedNiche" class="select"><option value="">Semua niche</option><option v-for="niche in niches" :key="niche.id" :value="niche.id">{{ niche.name }}</option></select><input v-model="search" class="input" placeholder="Cari konten..." /></div></div><p v-if="loading" class="muted">Memuat bank konten...</p><p v-if="error" class="error-box">{{ error }}</p><div v-else-if="items.length" class="content-list"><article v-for="item in items" :key="item.id" class="content-card"><div class="content-card-head"><span class="card-tags"><span class="status-pill">{{ item.status }}</span><span class="source-pill">{{ item.platform }}</span><span class="format-pill">{{ formatLabel(item) }}</span><span v-for="niche in (item.niches || []).slice(0, 2)" :key="niche.id" class="niche-mini">{{ niche.name }}</span></span><RouterLink :to="`/content-bank/${item.id}`" class="open-link">Detail →</RouterLink></div><h3>{{ itemTitle(item) }}</h3><p class="content-excerpt">{{ excerpt(item.original_text) }}</p><div class="popularity">{{ popularity(item) }}</div><div class="content-meta"><span>{{ item.author_handle || 'Author tidak dicatat' }}</span><span>{{ item.published_at ? new Date(item.published_at).toLocaleString('id-ID') : 'tanggal tidak dicatat' }}</span><span v-if="item.media?.length">{{ item.media.length }} media</span><span v-if="item.variants?.length">{{ item.variants.length }} varian</span></div><div class="content-card-actions"><RouterLink :to="`/content-bank/${item.id}`" class="button-secondary">Buka detail</RouterLink><button class="button-danger card-delete" type="button" @click="removeContent(item)">Hapus</button></div></article></div><p v-else-if="!loading" class="empty-state">Belum ada konten. Mulai dari halaman tangkap konten.</p></section>
  </div>
</template>

<style scoped>
.content-page{display:grid;gap:18px}.hero-row{display:flex;justify-content:space-between;align-items:end;gap:16px}.hero-row h1{font-size:48px;margin:0}.roadmap-badge,.status-pill,.source-pill{font:12px 'DM Mono';color:#1f6b4f;background:#e7f1e8;border-radius:999px;padding:7px 10px}.source-pill{background:#f2eee1;color:#7d6a3e}.content-form,.research-form{display:grid;gap:12px}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.custom-query-row{display:flex;gap:8px;align-items:center}.custom-query-row .input{flex:1}.custom-query-note{margin:0}.textarea{width:100%;resize:vertical;border:1px solid #cedbd0;border-radius:8px;padding:12px;background:#fffdf9;font:14px 'DM Mono';box-sizing:border-box}.label-group{display:grid;gap:8px;color:#52655a}.tag-options,.query-options,.research-actions{display:flex;flex-wrap:wrap;gap:8px}.tag-option,.query-chip{padding:8px 10px;border:1px solid #d9e2d8;border-radius:8px;background:#f8fbf7}.tag-option input{accent-color:#1f6b4f}.query-chip{cursor:pointer;color:#1f6b4f}.query-chip.selected{border-color:#1f6b4f;background:#e7f1e8}.query-chip:disabled{cursor:not-allowed;opacity:.55}.query-preview{margin:0;padding:10px 12px;border-radius:8px;background:#eef5ed;color:#52655a;overflow:auto}.query-preview code{font-family:'DM Mono';color:#1f6b4f}.filter-row{display:flex;gap:8px;flex-wrap:wrap}.filter-row .input{width:180px}.content-list{display:grid;gap:10px}.content-card{display:block;min-width:0;border:1px solid #d9e2d8;border-radius:10px;padding:16px;background:#fbfcf8;color:inherit;text-decoration:none;transition:transform .15s,border-color .15s}.content-card:hover{transform:translateY(-2px);border-color:#1f6b4f}.content-card-head{display:flex;justify-content:space-between;align-items:center;gap:12px;min-width:0}.card-tags{display:flex;align-items:center;gap:6px;min-width:0;flex-wrap:wrap}.niche-mini{font-size:12px;color:#52655a;background:#eef5ed;border-radius:999px;padding:7px 10px;max-width:180px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.open-link{color:#1f6b4f;font-weight:600;white-space:nowrap}.content-card h3{margin:14px 0 8px;font-size:20px;line-height:1.2;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden;word-break:break-word}.content-excerpt{margin:0;color:#52655a;line-height:1.45;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:3;overflow:hidden;word-break:break-word}.content-card small,.popularity{color:#78867c}.popularity{font-weight:600;margin:10px 0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.content-meta{display:flex;flex-wrap:wrap;gap:4px 12px;color:#78867c;font-size:12px;line-height:1.4}.empty-state{padding:30px;text-align:center;color:#78867c}.error-box{padding:10px;background:#fff0ed;color:#a84f43}.save-notice{color:#176b4f;font-weight:600}@media(max-width:700px){.form-grid,.hero-row{grid-template-columns:1fr;display:grid}.filter-row,.custom-query-row{flex-direction:column;align-items:stretch}.filter-row .input{width:auto}.content-card{padding:14px}.content-card-head{align-items:flex-start}.open-link{font-size:13px}}
</style>

<style scoped>
.content-card-actions{display:flex;justify-content:flex-end;align-items:center;gap:8px;margin-top:14px}.card-delete{padding:7px 10px;font-size:12px;line-height:1.2}
</style>

<style scoped>
.format-pill{font:12px 'DM Mono';color:#3e6381;background:#e8eef5;border-radius:999px;padding:7px 10px}
</style>
