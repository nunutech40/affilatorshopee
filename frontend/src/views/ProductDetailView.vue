...
watch(product, (p) => { if (p) { editText.value = getPromo(p); selectedContentModel.value = p.content_model || 'cheap' } })
...
<style scoped>.raw-pre{white-space:pre-wrap; max-height:320px; overflow:auto; color:#52655a; font:13px/1.6 'DM Mono'; background:#f9fbf8; padding:14px; border-radius:8px; border:1px solid #e1e8e0} .reformat-panel .mono{font:12px/1.5 'DM Mono'; min-height:420px} .muted{color:#78867c; font-size:13px} .section-heading{display:flex; justify-content:space-between; gap:10px; align-items:center} .reformat-actions{display:flex; gap:8px; align-items:end; margin:12px 0} .ai-loading{display:flex; align-items:center; gap:10px; margin:14px 0; padding:13px 16px; border-radius:8px; background:#e7f3eb; color:#176b4f; font-weight:600} .spinner{width:16px; height:16px; border:2px solid #b8d9c6; border-top-color:#176b4f; border-radius:50%; animation:spin .8s linear infinite} @keyframes spin{to{transform:rotate(360deg)}} .media-list{display:grid; gap:8px; margin-top:16px} .media-item{display:flex; justify-content:space-between; gap:12px; padding:10px; border-radius:7px; background:#e9f0e8; font-size:12px} .media-item small{color:#758379}</style>
