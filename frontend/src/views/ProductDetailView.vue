...
async function saveNiches() {
  ...
  try { await products.updateProductNiches(route.params.id, selectedNicheIDs.value); product.value = await products.getProduct(route.params.id); saveNotice.value = 'Jenis barang produk berhasil disimpan.' } catch (e) { error.value = e.message } finally { savingNiches.value = false }
}
...
function modelLabel(value) { return ({ trending: 'Trending', branded: 'Branded', cheap: 'Murah', curated: 'Curated', capture: 'Captured (legacy)' }[value] || 'Content model belum dipilih') }
