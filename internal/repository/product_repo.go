...
func (r *ProductRepository) Create(ctx context.Context, product *model.Product) error {
	query := `INSERT INTO products (
		raw_text, product_name, shopee_link, image_url, image_urls, video_url,
	...
	return r.db.QueryRowContext(ctx, query,
		product.RawText, product.ProductName, product.ShopeeLink, product.ImageURL,
		...
		pq.Array(product.HashtagPool), product.Notes, product.Status,
	).Scan(&product.ID, &product.CreatedAt, &product.UpdatedAt)
}

func (r *ProductRepository) GetByID(ctx context.Context, id string) (*model.Product, error) {
	query := `SELECT
		p.id, p.raw_text, p.product_name, p.shopee_link, p.image_url, p.image_urls,
		...
		p.urgency, p.caption_template, p.hashtag_pool, p.notes, p.status,
...
func (r *ProductRepository) update(ctx context.Context, product *model.Product, requiredStatus string) error {
	query := `UPDATE products SET
		product_name=$2, shopee_link=$3, image_url=$4, image_urls=$5, video_url=$6,
		normal_price=$7, sale_price=$8, discount_percent=$9, rating=$10,
		sold_count=$11, review_count=$12, keyword=$13, problem=$14, cluster=$15,
		content_model=$16, capture_angle=$17, benefit_1=$18, benefit_2=$19, benefit_3=$20,
		urgency=$21, caption_template=$22, hashtag_pool=$23, notes=$24, status=$25
	WHERE id=$1`
	args := []interface{}{
		product.ID, product.ProductName, product.ShopeeLink, product.ImageURL,
		pq.Array(product.ImageURLs), product.VideoURL, product.NormalPrice,
		product.SalePrice, product.DiscountPercent, product.Rating, product.SoldCount,
		product.ReviewCount, product.Keyword, product.Problem, product.Cluster,
		product.ContentModel, product.CaptureAngle, product.Benefit1, product.Benefit2,
	...
	if requiredStatus != "" {
		query = strings.TrimSuffix(query, "`") + " AND status=$26`"
...
func (r *ProductRepository) scanProduct(row interface {
	...
	var p model.Product
	err := row.Scan(
		&p.ID, &p.RawText, &p.ProductName, &p.ShopeeLink, &p.ImageURL,
		...
		pq.Array(&p.HashtagPool), &p.Notes, &p.Status, &p.CreatedAt, &p.UpdatedAt,
