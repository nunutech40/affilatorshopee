...
<template>
  <div class="product-list">
    ...
    <article v-for="product in items" :key="product.id" class="product-row">
      <label class="check"><input type="checkbox" :checked="selected.includes(product.id)" @change="emit('toggle', product.id)" /></label>
      <div class="row-main">
        <RouterLink :to="`/products/${product.id}`" class="row-title">{{ product.product_name || 'Raw product belum direformat' }}</RouterLink>
        <div class="row-sub">{{ product.cluster || 'uncategorized' }} · {{ (product.keyword || '').slice(0,32) }}</div>
      </div>
      <span class="badge model">{{ product.content_model || '-' }}</span>
