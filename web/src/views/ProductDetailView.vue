...
async function reformat() { working.value = true; error.value = ''; try { const result = await products.reformat([route.params.id], selectedModel.value); if (result.processed?.length) product.value = result.processed[0]; if (result.failed?.length) error.value = result.failed[0].message } catch (e) { error.value = e.message } finally { working.value = false } }
